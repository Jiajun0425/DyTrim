...
def get_cifar10(args, root):
    ...
    l_samples = make_imb_data(args.num_max, args.num_class, args.imb_ratio_label, 1, 0)
    u_samples = make_imb_data(args.num_max_u, args.num_class, args.imb_ratio_unlabel, 0, args.flag_reverse_LT)
...
def get_smallimagenet(args, root):
    ...
    base_test_dataset = SmallImageNet(root, args.img_size, True)
    ...
    num_class = len(set(base_dataset.targets))
    num_data_per_cls = [0 for _ in range(num_class)]
    for l in base_dataset.targets:
        num_data_per_cls[l] += 1

    test_num_class = len(set(base_test_dataset.targets))
    test_num_data_per_cls = [0 for _ in range(test_num_class)]
    for l in base_test_dataset.targets:
        test_num_data_per_cls[l] += 1
    ...
    # train_labeled_idxs = train_split(base_dataset.targets, num_labeled_data_per_cls, num_class, args.seed)
    ...
    return train_labeled_dataset, train_unlabeled_dataset, test_dataset, test_num_data_per_cls
