...
parser.add_argument('--out', default='out/cifar-10/N1500_M3000/fixmatchcdmad_100',
...
parser.add_argument('--img-size', default=32, type=int, help='image size for small imagenet')
...
parser.add_argument('--reverse', default=False, type=bool, help='whether to reverse the distribution of the unlabeled data')
parser.add_argument('--delta', default=0.875, type=float, help='Proportion of training on the pruned dataset')
...
if args.dataset == "smallimagenet":
    args.out = f"results/{args.dataset}/img_size{args.img_size}/fixmatch_{args.imb_ratio}_{args.imb_ratio_u}_{args.manualSeed}"
else: 
    args.out = f"results/{args.dataset}/N{args.num_max}_M{args.num_max_u}/fixmatch_{args.imb_ratio}_{args.imb_ratio_u}_{args.manualSeed}"
...
if args.dataset=='cifar10':
    ...
    from dataset.fix_smallimagenet import DATASET_GETTERS
    print(f'==> Preparing small imagenet 127')
    num_class = 127
    if args.img_size == 32:
        args.dataset_name = 'imagenet32_127'
    elif args.img_size == 64:
        args.dataset_name = 'imagenet64_127'
elif args.dataset=='imagenet':
    from dataset.fix_imagenet import load_data
    print(f'==> Preparing imbalanced ImageNet_LT')
    num_class = 1000
...
def compute_py(train_loader):
    """compute the base probabilities"""
    label_freq = {}
    for i, (inputs, labell, _, _) in enumerate(train_loader):
        labell = labell.cuda()
        for j in labell:
            key = int(j.item())
...
def main():
    ...
    U_SAMPLES_PER_CLASS = make_imb_data(args.num_max_u, num_class, args.imb_ratio_u,args.imbalancetype,args.reverse)
    ...
    if args.dataset == 'cifar10':
        ...
        train_labeled_set, train_unlabeled_set, test_set, _ = DATASET_GETTERS[args.dataset](args, './' + args.dataset_name)
    elif args.dataset=='imagenet':
        train_labeled_set = load_data('./imagenet', 'train')
        train_unlabeled_set = load_data('./imagenet', 'train', labelled=False)
        test_set = load_data('./imagenet', 'test')

    train_labeled_set = DyTrim(train_labeled_set, num_epoch=args.epochs, num_class=num_class, labelled=True, delta=args.delta)
    train_unlabeled_set = DyTrim(train_unlabeled_set, num_epoch=args.epochs, num_class=num_class, labelled=False, delta=args.delta)
    labeled_trainloader = data.DataLoader(train_labeled_set, batch_size=args.batch_size, shuffle=False, num_workers=4,
                                          drop_last=True, sampler=train_labeled_set.pruning_sampler())
    unlabeled_trainloader = data.DataLoader(train_unlabeled_set, batch_size=int(args.unlabeledratio*args.batch_size), shuffle=False, num_workers=4,drop_last=True, sampler=train_unlabeled_set.pruning_sampler())
    test_loader = data.DataLoader(test_set, batch_size=200, shuffle=False, num_workers=4)

    args.py_labeled = compute_py(labeled_trainloader)

    # Model
    print("==> creating WRN-28-2")

    def create_model(ema=False): 
        if args.dataset=='smallimagenet' or args.dataset=='imagenet': 
            import models.resnet as models
            model = models.ResNet50(num_classes=args.num_class, rotation=True, classifier_bias=True)
        else: 
    ...
    if args.resume:
        ...
        logger.set_names(['Top1_acc', 'Top1_acc_debias', 'GM', 'GM_debias', 'Best_Top1_acc'])
    labeled_train_iter = unlabeled_train_iter = None
    for epoch in range(start_epoch, args.epochs):
        print('\nEpoch: [%d | %d] LR: %f' % (epoch + 1, args.epochs, state['lr']))

        
        labeled_train_iter, unlabeled_train_iter, _ = train(labeled_train_iter, unlabeled_train_iter, train_labeled_set, train_unlabeled_set, labeled_trainloader,unlabeled_trainloader,model, optimizer,ema_optimizer,train_criterion,epoch)
    ...
    logger.close()
def train(labeled_train_iter, unlabeled_train_iter, train_labeled_set, train_unlabeled_set, labeled_trainloader,unlabeled_trainloader, model,optimizer, ema_optimizer, criterion, epoch):
    ...
    train_labeled_set.__setepoch__(epoch)
    train_unlabeled_set.__setepoch__(epoch)
    ...
    for batch_idx in range(args.val_iteration):
        ...
        try:
            inputs_x, targets_x, idx, weight = next(labeled_train_iter)
        except:
            labeled_train_iter = iter(labeled_trainloader)
            inputs_x,  targets_x, idx, weight = next(labeled_train_iter)

        try:
            (inputs_u, inputs_u2, inputs_u3), _, idx_u, weight_u = next(unlabeled_train_iter)
        except:
            unlabeled_train_iter = iter(unlabeled_trainloader)
            (inputs_u, inputs_u2, inputs_u3), _, idx_u, weight_u = next(unlabeled_train_iter)
        ...
        weight, weight_u = weight.cuda(), weight_u.cuda()

        with torch.no_grad():
            black = torch.zeros((1, 3, 32, 32)).cuda()
            biaseddegree, _ = model(black)
            outputs_u, _ = model(inputs_u)
            if epoch>args.debiasstart:
                outputs_u = outputs_u - biaseddegree.detach()
                train_labeled_set.__setbiaseddegree__(F.softmax(biaseddegree).detach().cpu().numpy())
            else:
                train_labeled_set.__setbiaseddegree__(args.py_labeled.detach().cpu().numpy())
        ...
        train_labeled_set.__setscore__(idx.detach().cpu().numpy(), Lx.detach().cpu().numpy())
        train_unlabeled_set.__setscore__(idx_u.detach().cpu().numpy(), (Lu[:batch_size_u]+Lu[batch_size_u:]).detach().cpu().numpy())
        Lx = torch.mean(Lx*weight)
        Lu = torch.mean(Lu*torch.cat([weight_u, weight_u], dim=0))
    ...
    return labeled_train_iter, unlabeled_train_iter, (losses.avg, losses_x.avg, losses_u.avg)
...
def validate(valloader, model, criterion, mode):
    ...
    if args.dataset=='cifar10':
        ...
        accperclass2 = accperclass2 / 1500
    elif args.dataset=='cifar100':
...
def make_imb_data(max_num, class_num, gamma, imb, reverse=False):
    if imb == 'long':
...
class SemiLoss(object):
    def __call__(self, outputs_x, targets_x, outputs_u, targets_u, mask):
        Lx = -(torch.sum(torch.log(F.softmax(outputs_x, dim=1)+1e-8) * targets_x, dim=1))
        Lu = -(torch.sum(torch.log(F.softmax(outputs_u, dim=1)+1e-8) * targets_u, dim=1) * mask)
        return Lx, Lu
